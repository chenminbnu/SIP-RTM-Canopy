...
for i_LAI=1:70
    ...
    try
        ...
        ind=ind+1;
        RSO_LUT_Cropland(ind,1:5)=[BRF2 A i0];
    catch exception
        disp(exception);
    end
end

RSO_LUT_Cropland(:,6)=(RSO_LUT_Cropland(:,2)-RSO_LUT_Cropland(:,1))./(RSO_LUT_Cropland(:,2)+RSO_LUT_Cropland(:,1)).*RSO_LUT_Cropland(:,2);
save('RSO_LUT_Cropland.mat','RSO_LUT_Cropland','-v7.3');
