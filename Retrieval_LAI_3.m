...
fileOutpath=['/pic/projects/sif/dalei/EPIC-data/Estimate LAI/',Tm{hi},'/'];
...
tic
    ...
    for row = 1:rows
        for col = 1:cols
            ...
            filter = (~isnan( Red_tmp)) & (~isnan( NIR_tmp))& (~isnan( SZA_tmp))& (~isnan( VZA_tmp))& (~isnan( RAA_tmp));
         if(sum(filter)>0)
...
toc
    save([fileOutpath '2016-d' num2str(t,'%03d') '.mat'],'Results');
disp('finished!');
